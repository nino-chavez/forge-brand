 ...
 * Init Command
 *
 * brand-forge init [--from <preset>] [--from-exploration <dir>]
...
import { extractFromExploration, type PartialBrandKit } from '../../core/extractors/index.js';
...
export function registerInitCommand(program: Command) {
  program
    ...
    .option('--from-exploration <dir>', 'Extract brand from a website-exploration variant directory')
    .option('-o, --output <path>', 'Output path', 'brand-kit.json')
    .action(async (options: { from?: string; fromExploration?: string; output: string }) => {
      console.log(chalk.bold('\nbrand-forge — Brand Discovery\n'));

      let kit: Record<string, unknown>;

      if (options.fromExploration) {
        // Extract from website-exploration variant
        const dirPath = path.resolve(options.fromExploration);
        if (!fs.existsSync(dirPath) || !fs.statSync(dirPath).isDirectory()) {
          console.error(chalk.red(`Directory not found: ${dirPath}`));
          process.exit(1);
        }

        console.log(chalk.dim(`Extracting from exploration variant: ${dirPath}\n`));
        const partial = extractFromExploration(dirPath);
        printExtractionSummary(partial);
        kit = await fillGaps(partial, dirPath);
...
function printExtractionSummary(partial: PartialBrandKit) {
  console.log(chalk.bold.underline('Extraction Results\n'));
  ...
  if (partial.colors) {
    ...
    console.log(`${check} Colors: ${c.allExtracted.length} unique hex values found`);
    if (c.primary) console.log(chalk.dim(`    primary: ${c.primary.hex}`));
    if (c.secondary) console.log(chalk.dim(`    secondary: ${c.secondary.hex}`));
    if (c.background) console.log(chalk.dim(`    background: ${c.background}`));
    if (c.foreground) console.log(chalk.dim(`    foreground: ${c.foreground}`));
  } else {
    console.log(`${miss} Colors: none found`);
  }

  // Typography
  if (partial.typography) {
    const t = partial.typography;
    console.log(`${check} Typography:`);
    if (t.display) console.log(chalk.dim(`    display: ${t.display.family} (${t.display.classification})`));
    if (t.body) console.log(chalk.dim(`    body: ${t.body.family} (${t.body.classification})`));
    if (t.mono) console.log(chalk.dim(`    mono: ${t.mono.family} (${t.mono.classification})`));
  } else {
    console.log(`${miss} Typography: none found`);
  }

  // Mode
  if (partial.mode) {
    console.log(`${check} Theme mode: ${partial.mode}`);
  } else {
    console.log(`${miss} Theme mode: could not infer`);
  }

  // Layout
  if (partial.layout?.maxWidth) {
    console.log(`${check} Layout: max-width ${partial.layout.maxWidth}rem`);
  } else {
    console.log(`${miss} Layout: no max-width found`);
  }

  // Spacing
  if (partial.spacing) {
    console.log(`${check} Spacing: ${partial.spacing.values.length} values found`);
  } else {
    console.log(`${miss} Spacing: none found`);
  }

  // Voice hints
  if (partial.voiceHints) {
    const v = partial.voiceHints;
    const total = v.headings.length + v.ctaLabels.length + v.bodySnippets.length;
    console.log(`${check} Voice hints: ${total} text snippets (${v.headings.length} headings, ${v.ctaLabels.length} CTAs, ${v.bodySnippets.length} body)`);
  } else {
    console.log(`${miss} Voice hints: no copy text found`);
  }
...
// ---------------------------------------------------------------------------
// Gap-Filling: Confirm extracted values, prompt for missing ones
// ---------------------------------------------------------------------------

async function fillGaps(partial: PartialBrandKit, _dirPath: string): Promise<Record<string, unknown>> {
  // --- Identity (always prompt — can't extract brand identity from code) ---
  ...
  // --- Theme Mode ---
  let defaultMode: 'dark' | 'light';
  if (partial.mode) {
    const confirm = await select({
      message: `Detected ${partial.mode} mode — use this?`,
      choices: [
        { name: `Yes (${partial.mode})`, value: partial.mode },
  ...
  if (partial.colors?.primary) {
    primaryHex = await input({
      ...
      default: partial.colors.primary.hex,
    });
  } else {
    console.log(chalk.dim('Enter 6-digit hex colors with # prefix (e.g. #facc15)\n'));
    primaryHex = await input({ message: 'Primary color hex:' });
  }
  primaryName = await input({ message: 'Primary color name:' });

  if (partial.colors?.secondary) {
    secondaryHex = await input({
      message: 'Secondary color hex:',
      default: partial.colors.secondary.hex,
    });
  } else {
    secondaryHex = await input({ message: 'Secondary color hex:' });
  }
  secondaryName = await input({ message: 'Secondary color name:' });
  ...
  const bgDefault = partial.colors?.background ?? (defaultMode === 'dark' ? '#0a0a0a' : '#fafaf9');
  const fgDefault = partial.colors?.foreground ?? (defaultMode === 'dark' ? '#ffffff' : '#1c1917');
  const background = await input({ message: 'Background color:', default: bgDefault });
  const foreground = await input({ message: 'Foreground (text) color:', default: fgDefault });

  // --- Typography ---
  console.log(chalk.bold.underline('\nTypography'));

  const displayDefault = partial.typography?.display?.family ?? 'Inter';
  const displayFont = await input({ message: 'Display font family:', default: displayDefault });

  const displayClassDefault = partial.typography?.display?.classification ?? 'sans-serif';
  const displayClass = await select({
    message: 'Display font classification:',
    default: displayClassDefault,
    choices: [
  ...
  const bodyDefault = partial.typography?.body?.family ?? 'Inter';
  const bodyFont = await input({ message: 'Body font family:', default: bodyDefault });

  const monoDefault = partial.typography?.mono?.family ?? 'JetBrains Mono';
  const monoFont = await input({ message: 'Mono font family:', default: monoDefault });
  ...
  return {
    ...
    typography: {
      ...
      body: {
        family: bodyFont,
        fallbacks: ['system-ui', 'sans-serif'],
        classification: partial.typography?.body?.classification ?? 'sans-serif',
        weights: [400, 500, 600, 700],
        variable: true,
    ...
    layout: {
      maxContentWidth: partial.layout?.maxWidth ?? 80,
      breakpoints: { sm: 640, md: 768, lg: 1024, xl: 1280, '2xl': 1536 },
      sectionPadding: partial.spacing?.sectionPadding ?? '16',
      containerPadding: '6',
    },
    voice: {
...
// ---------------------------------------------------------------------------
// Interactive Discovery Flow (original)
// ---------------------------------------------------------------------------
